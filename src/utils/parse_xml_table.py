...
def extract_rows(root: ET.Element, max_columns: int) -> List[str]:
    ...
    if tbody is not None:
        for row in tbody.findall("tr"):
            cols = [
                unescape("".join(col.itertext()).strip()) for col in row.findall("td")
            ]
            # Fill in empty cells with empty str
...
def parse_table(xml_content: str) -> dict:
    ...
    table_data = {"table_headers": [], "table_subheaders": [], "table_rows": []}

    max_columns = get_max_columns(root)
    table_data["table_headers"], table_data["table_subheaders"] = extract_headers(
        root, max_columns
    )
...
def parsed_tab_xml_df(
    row: pd.Series, condition_col: str, condition_value: str, xml_col: str
) -> pd.Series:
    """
    ...
    if row[condition_col] == condition_value:
        ...
        return pd.Series(
            {"table_headers": None, "table_subheaders": None, "table_rows": None}
        )
